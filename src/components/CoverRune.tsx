...
export type CoverRuneVariant = 'front' | 'back'

export type CoverRuneProps = {
  /** `front` : interactif ; `back` : décoratif, symétrie miroir. */
  variant?: CoverRuneVariant
}

/**
 * Rune / sigil sur la couverture. Avant : animation + survol + clic.
 * Dos : même glyphe en miroir, sans interaction.
 */
export function CoverRune({ variant = 'front' }: CoverRuneProps) {
  ...
  const tiltStyle: CSSProperties = isBack
    ? { transform: 'perspective(380px) scaleX(-1)' }
    : {
  ...
  if (isBack) {
    return (
      <div
        className="cover-rune cover-rune--back"
        aria-hidden
      >
